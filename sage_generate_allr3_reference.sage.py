...
# This file was *autogenerated* from the file sage_generate_allr3_reference.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_11 = Integer(11)
...
def _all_subsets(E):
    ...
    E = list(E)
    n = len(E)
    for mask in range(_sage_const_1  << n):
        S = set()
        for i, e in enumerate(E):
            if (mask >> i) & _sage_const_1 :
                S.add(e)
        yield S
...
def uniform_r3n3_if_needed(n, mats):
    ...
    Inject it if needed.
    """
    if n == _sage_const_3  and len(mats) == _sage_const_0 :
        return [matroids.Uniform(_sage_const_3 , _sage_const_3 )]
    return mats
...
def nondegenerate_flats_inequalities_rank3(M):
    ...
    for S in _all_subsets(E):
        if len(S) == _sage_const_0  or len(S) == n:
            continue
    ...
    out = sorted(set(out), key=lambda t: (t[_sage_const_1 ], t[_sage_const_0 ]))
    return out
...
def sha_volume_rank3(M):
    ...
    for i in range(n):
        for j in range(i + _sage_const_1 , n):
            if int(M.rank({E[i], E[j]})) == _sage_const_1 :
                union(i, j)
    ...
    flats2 = set()
    for a_i in range(d):
        for b_i in range(a_i + _sage_const_1 , d):
            a = reps[a_i]
            b = reps[b_i]
            if int(M.rank({a, b})) < _sage_const_2 :
                continue
            cl = _closure(M, {a, b}, Eset)
            cl_classes = frozenset(class_of[e] for e in cl)
            if len(cl_classes) >= _sage_const_2 :
                flats2.add(cl_classes)

    sub = _sage_const_0 
    for F in flats2:
        m_p = len(F)
        if m_p >= _sage_const_3 :
            sub += (m_p - _sage_const_2 ) * (m_p - _sage_const_2 )

    vol = (d - _sage_const_3 ) * (d - _sage_const_3 ) - sub
    return int(vol)
...
def _parse_n_arg(argv):
    ...
    for i, a in enumerate(argv):
        if a in ('--n', '-n') and i + _sage_const_1  < len(argv):
            try:
                n = int(argv[i + _sage_const_1 ])
                return n
            except Exception:
                pass
...
def main():
    n_arg = _parse_n_arg(sys.argv[_sage_const_1 :])
    if n_arg is None:
        ns = range(_sage_const_3 , _sage_const_11 )
    else:
        ns = [n_arg]
    for n in ns:
        ...
        mats = uniform_r3n3_if_needed(n, list(matroids.AllMatroids(n, _sage_const_3 )))

        recs = []
        orbit_id = _sage_const_0 
        seen_bases = set()
